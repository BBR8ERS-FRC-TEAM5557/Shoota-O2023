...
public class SwerveFFCharacterization extends CommandBase {
    ...
    public SwerveFFCharacterization(Subsystem subsystem, boolean forwards,
    ...
    public void execute() {
        if (timer.get() < startDelaySecs) {
            ...
            double voltage =
                    (timer.get() - startDelaySecs) * rampRateVoltsPerSec * (forwards ? 1 : -1);
            if (isDrive) {
